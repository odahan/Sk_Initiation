...
namespace SKNativePlugins.Plugins;

public class OutilsTexte
{
    [KernelFunction("compterMots")]
    public int Compter(string texte) =>
        string.IsNullOrWhiteSpace(texte) ? 0 :
            texte.Split(' ', StringSplitOptions.RemoveEmptyEntries).Length;

    [KernelFunction("tronquer")]
    public string Tronquer(string texte, int longueur) =>
        string.IsNullOrWhiteSpace(texte) || texte.Length <= longueur
            ? texte
            : texte.Substring(0, longueur) + "...";

    [KernelFunction("inverser")]
    public string Inverser(string texte) =>
        new string(texte?.Reverse().ToArray() ?? Array.Empty<char>());
}