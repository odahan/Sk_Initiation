...
// Plugin natif
var plugin = kernel.ImportPluginFromObject(new AnalyseTexte(), "analyse");

// Lecture de l’input utilisateur
Console.Write("Entrez un texte à analyser : ");
var inputTexte = Console.ReadLine();

// Seuil fixé
int threshold = 10;

// Création du sac partagé
var sharedArgs = new KernelArguments
{
    ["input"] = inputTexte!,
    ["threshold"] = threshold
};

// Récupération des KernelFunctions
var compterFn = plugin["countCharacters"]; // les accentuées ne sont pas acceptées
var evaluerFn = plugin["estimateLength"];  // alors préférons les noms anglais...

// Étape 1 – Compter les caractères
var nbCaractères = await compterFn.InvokeAsync(kernel, sharedArgs);
sharedArgs["charCount"] = nbCaractères.GetValue<int>();

// Étape 2 – Évaluer la longueur
var évaluation = await evaluerFn.InvokeAsync(kernel, sharedArgs);

// Affichage
Console.WriteLine($"\nNombre de caractères : {sharedArgs["charCount"]}");
Console.WriteLine($"Évaluation : {évaluation.GetValue<string>()}");