...
public static class ApiKeyStore
{
    private static readonly string folder = Path.Combine(
        ...
        "YourCompanyOrAppName");
    private static readonly string path = Path.Combine(folder, "apikeyGen.dat");

    public static void Save(string key)
    {
        Directory.CreateDirectory(folder);
        var obfuscated = Convert.ToBase64String(Encoding.UTF8.GetBytes(key));
