...
// Import du plugin sémantique
var pluginPath = Path.Combine(Directory.GetCurrentDirectory(), "Plugins", "AssistantSupport");

var plugin = SemanticPluginLoader.LoadSemanticPlugin(kernel, pluginPath, out var pluginName);

foreach (var entry in plugin)
{
    Console.WriteLine($"Fonction chargée : {entry.Key}");
}

// Import du plugin C# natif
KernelPlugin pluginUtils = kernel.ImportPluginFromObject(new FonctionsUtiles(), "utils");

// Récupération des fonctions
var dateFr = pluginUtils["dateFr"];

// Appel des fonctions
var result = await kernel.InvokeAsync(plugin["Resumeur"], new()
{
    ["input"] = "Semantic Kernel est une librairie .NET conçue pour orchestrer l’IA générative et la logique métier."
});

var dateResult = await kernel.InvokeAsync(dateFr);

// Affichage
Console.WriteLine($"Résumé généré : {result}");
Console.WriteLine($"Date du jour : {dateResult.GetValue<string>()}");